					...
					<div class="row">
						...
						<div class="col-xl-5 col-lg-6 col-md-12 col-sm-12">
							...
							@if(Auth::guard('customer')->id())
								 <a href="{{ route('profile_info') }}">Mr. {{ Auth::guard('customer')->user()->name }}</a>
								@else
								<a href="{{ route('login.register') }}" class="text-muted medium"><i class="lni lni-user mr-1"></i>Sign In / Register</a>
							@endif
							</div>
						</div>
									...
									<ul class="nav-menu nav-menu-social align-to-right align-items-center d-flex">
										...
										<li>
											<a href="#" onclick="openCart()">
												...
												@endphp
													<i class="fas fa-shopping-basket fs-lg"></i><span class="dn-counter theme-bg">{{ $carts->count() }}</span>
												</div>
											</a>
										</li>
									</ul>	
					...
					<div class="right-ch-sideBar">
						...
						<div class="cart_select_items py-2">
							...
							<div class="d-flex align-items-center justify-content-between br-bottom px-3 py-3">
								<div class="cart_single d-flex align-items-center">
									<div class="cart_selected_single_thumb">
										<a href="#"><img src="{{asset('uploads/product/preview/')}}/{{ $cart->product->preview }}" width="60" class="img-fluid" alt="" /></a>
									</div>
									<div class="cart_single_caption pl-2">
										<h4 class="product_title fs-sm ft-medium mb-0 lh-1">{{$cart->product->product_name}}</h4>
										<p class="mb-2"><span class="text-dark ft-medium small">{{ $cart->size->size_name }}</span>, <span class="text-dark small">{{ $cart->color->color_name }}</span></p>
										<h4 class="fs-md ft-medium mb-0 lh-1"><span class="p-3">{{ $cart->quantity }} X</span> &#2547; {{ $cart->product->after_discount }}</h4>
									</div>
								</div>
